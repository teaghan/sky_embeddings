...
def main(args):
    ...
    model, losses, cur_iter, optimizer, lr_scheduler = build_model(  # type: ignore
        config, model_filename, device, build_optimizer=True
    )
    ...
    if 'survey' in config['DATA'] and config['DATA']['survey'] == 'UNIONS':
        ...
        dataloader_val = build_unions_dataloader(
            batch_size=int(config['TRAINING']['batch_size']),
            num_workers=num_workers,
            patch_size=int(config['ARCHITECTURE']['patch_size']),
            num_channels=int(config['ARCHITECTURE']['num_channels']),
        ...
        if lp_regress_data_file is not None:
            dataloader_regress = build_unions_dataloader(
                batch_size=int(config['TRAINING']['batch_size']),
                num_workers=num_workers,
                patch_size=int(config['ARCHITECTURE']['patch_size']),
                num_channels=int(config['ARCHITECTURE']['num_channels']),
        ...
        if lp_class_data_file is not None:
            dataloader_classfication = build_unions_dataloader(
                batch_size=int(config['TRAINING']['batch_size']),
                num_workers=num_workers,
                patch_size=int(config['ARCHITECTURE']['patch_size']),
                num_channels=int(config['ARCHITECTURE']['num_channels']),
        ...
        if 'train_data_file' in config['DATA']:
            ...
            dataloader_train = build_fits_dataloader(
                eval(config['DATA']['train_data_paths']),
        ...
        dataloader_val = build_h5_dataloader(
            os.path.join(data_dir, config['DATA']['val_data_file']),
            batch_size=int(config['TRAINING']['batch_size']),
            num_workers=num_workers,
            patch_size=int(config['ARCHITECTURE']['patch_size']),
            num_channels=int(config['ARCHITECTURE']['num_channels']),
    ...
    while cur_iter < (total_batch_iters):
        ...
        for samples, masks, ra_decs in get_train_samples(dataloader_train, train_nested_batches):
            ...
            if cur_iter % verbose_iters == 0:
                ...
                plot_batch(
                    orig_imgs[interesting_val_idx],
                    mask_imgs[interesting_val_idx],
                    pred_imgs[interesting_val_idx],
                    n_samples=len(interesting_val_idx),
                    channel_index=0,
                    savename=os.path.join(
                        fig_dir,
