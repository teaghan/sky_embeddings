...
from utils import cutouts_to_hdf5, get_cutouts_from_patch, getPatches
...
def create_h5_dataset(
    ...
    if (labels_path is not None) & (not create_grid):
        ...
        labels = pd.read_csv(labels_path)  # type: ignore
    ...
    for i in range(patch_start, patch_end, batch_size):
        ...
        if os.path.exists(os.path.join(out_dir, out_filename)):
            ...
            create_h5_dataset(
                ...
                patch_strategy='n_to_n',
                patch_low_bound=low,
                patch_high_bound=high,
            )
...
out_dir = '/home/heesters/projects/def-sfabbro/heesters/github/sky_embeddings/data/'
...
if 'gal' in file_type:
    # Galaxies
    for out_name, use_calexp in zip(
        ['HSC_dud_galaxy_GIRYZ7610_64'],  #'HSC_dud_galaxy_calexp_GIRYZ7610_64',
        [False],
    ):  # True
...
if 'qso' in file_type:
    # QSOs
    for out_name, use_calexp in zip(
        [f'HSC_dud_qso_calexp_GIRYZ7610_{img_size}', f'HSC_dud_qso_GIRYZ7610_{img_size}'], [True, False]
    ):
...
if 'star' in file_type:
    # Stars
    for out_name, use_calexp in zip(
        [f'HSC_dud_star_calexp_GIRYZ7610_{img_size}', f'HSC_dud_star_GIRYZ7610_{img_size}'],
        [True, False],
    ):
...
if 'zspec' in file_type:
    ...
    for out_name, use_calexp in zip(
        [
            f'HSC_dud_galaxy_zspec_calexp_GIRYZ7610_{img_size}_train',
            f'HSC_dud_galaxy_zspec_GIRYZ7610_{img_size}_train',
        ],
        [True, False],
    ):
    ...
    for out_name, use_calexp in zip(
        [
            f'HSC_dud_galaxy_zspec_calexp_GIRYZ7610_{img_size}_val',
            f'HSC_dud_galaxy_zspec_GIRYZ7610_{img_size}_val',
        ],
        [True, False],
    ):
    ...
    for out_name, use_calexp in zip(
        [
            f'HSC_dud_galaxy_zspec_calexp_GIRYZ7610_{img_size}_test',
            f'HSC_dud_galaxy_zspec_GIRYZ7610_{img_size}_test',
        ],
        [True, False],
    ):
...
if 'dwarf' in file_type:
    ...
    for out_name, use_calexp in zip(
        [
            f'HSC_dud_dwarf_galaxy_calexp_GIRYZ7610_{img_size}',
            f'HSC_dud_dwarf_galaxy_GIRYZ7610_{img_size}',
        ],
        [True, False],
    ):
...
if 'lense' in file_type:
    ...
    for out_name, use_calexp in zip(
        [
            f'HSC_dud_strong_lens_calexp_GIRYZ7610_{img_size}',
            f'HSC_dud_strong_lens_GIRYZ7610_{img_size}',
        ],
        [True, False],
    ):
...
if 'unknown' in file_type:
    # Stars
    for out_name, use_calexp in zip(
        [f'HSC_dud_unknown_calexp_GIRYZ7610_{img_size}', f'HSC_dud_unknown_GIRYZ7610_{img_size}'],
        [True, False],
    ):
