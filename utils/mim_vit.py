...
def build_model(config, model_filename, device, build_optimizer=False):
    ...
    if model_type == 'base':
        ...
        model = mae_vit_base_with_simple_decoder(
            embed_dim=embed_dim,
            img_size=img_size,
            in_chans=num_channels,
            patch_size=patch_size,
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(
        ...
        if self.ra_dec:
            ...
            self.ra_dec_embed = LocationEncoder(
                neural_network_name='siren',
                legendre_polys=5,
                dim_hidden=8,
                num_layers=1,
        ...
        if self.simmim:
            ...
            self.decoder = nn.Sequential(
                nn.Conv2d(
                    ...
                    out_channels=dec_upsample_size**2 * in_chans,
                    kernel_size=1,
                ),
    ...
    def forward_features(self, x, ra_dec=None, mask_ratio=0.0, mask=None, reshape_out=True):
        B, C, H, W = x.shape
        ...
        if self.attn_pool:
            x = self.attn_pool(x).unsqueeze(1)  # type: ignore
    ...
    def forward(self, imgs, ra_dec=None, mask_ratio=0.75, mask=None, denorm_out=False):
        latent, mask, ids_restore = self.forward_features(
            imgs, ra_dec=ra_dec, mask_ratio=mask_ratio, mask=mask
        )
...
def patch_mean_and_var(imgs):
    ...
    mean = torch.where(non_nan_mask, imgs, torch.tensor(0.0, device=imgs.device)).sum(
        dim=-1, keepdim=True
    ) / non_nan_mask.sum(dim=-1, keepdim=True)
...
def undo_pixel_norm(original_images, normalized_images, model):
    ...
    unnormalized = normalized_images * (var + 1.0e-6) ** 0.5 + mean
