...
def build_fits_dataloader(fits_paths, bands, min_bands, batch_size, num_workers,
                          ...
                          augment=False, shuffle=True, transforms=None, collator=None):
...
def build_h5_dataloader(filename, batch_size, num_workers, 
                        label_keys=None, img_size=64, pos_channel=False, 
                        augment=False, shuffle=True, indices=None, transforms=None, collator=None):
    ...
    # Build dataset
    dataset = H5Dataset(filename, img_size=img_size,
...
class H5Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_file, img_size, 
                 pos_channel=False, label_keys=None, 
...
class FitsDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, fits_paths,  bands=['G','R','I','Z','Y'], min_bands=5,
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            for i in range(len(cutouts)):
                cutouts[i] = self.transform(cutouts[i])
