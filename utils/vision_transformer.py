...
from utils.masks import apply_masks
...
def build_model(config, model_filename, device, build_optimizer=False):
    ...
    if model_type=='tiny':
        encoder = vit_tiny(img_size=[img_size],
                             in_chans=num_channels,
                             patch_size=patch_size,
                             pixel_mean=pixel_mean,
                             pixel_std=pixel_std)
    elif model_type=='small':
        encoder = vit_small(img_size=[img_size],
                              in_chans=num_channels,
                              patch_size=patch_size,
                              pixel_mean=pixel_mean,
                              pixel_std=pixel_std)
    elif model_type=='base':
        encoder = vit_base(img_size=[img_size],
                             in_chans=num_channels,
                             patch_size=patch_size,
                             pixel_mean=pixel_mean,
                             pixel_std=pixel_std)
    elif model_type=='large':
        encoder = vit_large(img_size=[img_size],
                           in_chans=num_channels,
                           patch_size=patch_size,
                           pixel_mean=pixel_mean,
                           pixel_std=pixel_std)
    elif model_type=='huge':
        encoder = vit_huge(img_size=[img_size],
                           in_chans=num_channels,
                           patch_size=patch_size,
                           pixel_mean=pixel_mean,
                           pixel_std=pixel_std)
    ...
    if build_optimizer:
        ...
        warmup = 0.02
        ema = eval(config['TRAINING']['ema'])

        optimizer, lr_scheduler, wd_scheduler = init_opt(
                                                        encoder,
                                                        predictor,
                                                        total_batch_iters,
                                                        start_lr,
                                                        ...
                                                        final_lr=final_lr,
                                                        use_bfloat16=False)
        losses, cur_iter = load_model(encoder, predictor, model_filename, optimizer, lr_scheduler, wd_scheduler)

        # -- momentum schedule
        #momentum_scheduler = (ema[0] + i*(ema[1]-ema[0])/(total_batch_iters)
        #                      for i in range(int(total_batch_iters)+1))
        momentum_scheduler = {'ema': ema, 'total_batch_iters':total_batch_iters}

        
        return encoder, predictor, losses, cur_iter, optimizer, lr_scheduler, wd_scheduler, momentum_scheduler
    else:
        losses, cur_iter = load_model(encoder, predictor, model_filename)
        return encoder, predictor, losses, cur_iter
...
def init_opt(
    ...
    lr_scheduler = WarmupCosineSchedule(
        optimizer,
        warmup_steps=int(warmup*total_batch_iters),
        start_lr=start_lr,
        ref_lr=ref_lr,
        final_lr=final_lr,
        T_max=int(total_batch_iters))
...
def load_model(encoder, predictor, model_filename, optimizer=None, 
               lr_scheduler=None, wd_scheduler=None, target_encoder=None):
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.tile_size = (img_size[0])//(patch_size)
