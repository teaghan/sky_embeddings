...
def parseArguments():
    ...
    parser.add_argument("-tst_dirs", "--test_dirs",
                    type=str, nargs='+', default=['/project/rrg-kyi/astro/hsc/pdr3_dud/'])
...
test_dataloader =  build_fits_dataloader(test_dirs, 
...
test_images, test_latent, test_ra_decs, test_scores = mae_simsearch(model, target_latent, test_dataloader, 
                               ...
                               max_pool=max_pool, cls_token=cls_token, nested_batches=True, n_save=n_save)
...
# Save results
np.savez(os.path.join(results_dir, f'{model_name}_{target_fn[:-3]}_simsearch_results.npz'), 
